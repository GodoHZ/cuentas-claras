...
def as_dict(conn, exported_at: str, version: str) -> dict:
    return {
        ...
        "cuentas": [{**dict(r), "initial_balance": _euros(r["initial_balance"])} for r in repo.accounts(conn)],
        ...
        "cuadres": [{**dict(r), "balance": _euros(r["balance"])}
                    for r in conn.execute("SELECT * FROM account_checks ORDER BY date, id")],
    }
...
def to_csv_zip(conn) -> bytes:
    ...
    with zipfile.ZipFile(out, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        z.writestr("sobres.csv", _csv(
            ["Nombre", "Objetivo (€)", "Aporte mensual (€)", "Cuenta", "Nota", "Orden", "Activo"],
            [[r["name"], amount_input(r["target"]), amount_input(r["monthly"]),
              cuentas.get(r["account_id"], ""), r["note"], r["position"],
              "sí" if r["active"] else "no"] for r in repo.envelopes(conn)]))
        z.writestr("cuentas.csv", _csv(
            ["Nombre", "Saldo inicial (€)", "Orden", "Activa"],
            [[r["name"], amount_input(r["initial_balance"]), r["position"],
              "sí" if r["active"] else "no"] for r in repo.accounts(conn)]))
        ...
        z.writestr("cuadres.csv", _csv(
            ["Fecha", "Cuenta", "Saldo real (€)"],
            [[fmt_date(r["date"]), r["account"] or "", amount_input(r["balance"])]
             for r in conn.execute("SELECT k.*, a.name AS account FROM account_checks k LEFT JOIN accounts a ON a.id = k.account_id ORDER BY k.date, k.id")]))
    return out.getvalue()
